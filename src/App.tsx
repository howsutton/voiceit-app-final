...
const KioskMode = ({ project, sessionTimeout, onExit }: { project: Project, sessionTimeout: number, onExit: () => void }) => {
  ...
  const openSource = useCallback((source: any, sourceSet?: any[]) => {
    ...
    for (const s of sourcesToPreview) {
      ...
      if (!seen.has(key)) {
        seen.add(key);
        finalSources.push({
          id: 'source-' + generateId() + '-' + finalSources.length,
  ...
  const getFullGreeting = useCallback(() => {
    return `Welcome. I am VoiceIt, your AI assistant for this project - ${project.title}, and I am here to help you quickly find information from the uploaded source documents. I can answer your questions, show you where my answers come from, list all available sources, and read source passages aloud. You can talk with me naturally or type your questions instead. When your session is complete, I can also prepare a summary and QR code so you can keep a record of what we covered. If you need help at any time or would like to know what I can do, just say 'Help.'`;
  }, [project.title]);
  ...
  const connectLive = async (currentDocs?: Document[], isReturn: boolean = false, initialHistory?: Message[]) => {
    ...
    try {
      ...
      const systemInstruction = `
        ...
        Introduce yourself using the EXACT FULL GREETING below. Do not paraphrase, shorten, or prepend any other text.
        ...
        1. When you receive a "NEW_PRESENCE" signal, immediately respond with the FULL GREETING.
        2. When you receive a "RETURN_PRESENCE" signal, if you have not already delivered the FULL GREETING in this session, deliver it now. Otherwise, respond with a shorter welcome back message.
        3. Do not wait for the visitor to say hello first.
        4. Do not ask for permission to begin.
        5. Keep the greeting natural, polished, and professional.
        6. HELP REQUESTS: If the user asks for help, says "Help", or asks what you can do, respond with a short capability summary (e.g., "I can answer questions from your documents, show sources, and read them aloud. How can I help?"). Do NOT repeat the full greeting for help requests unless it is the very first interaction.
        ...
        callbacks: {
          onopen: () => {
            ...
            if (initialHistory && initialHistory.length > 0) {
              ...
              const promptText = (isReturn && hasGreetedRef.current)
                ? `[SIGNAL: ${signal}] The user has returned. Please greet them back warmly.` 
                : `[SIGNAL: ${signal}] A new session has started. Please introduce yourself using the EXACT FULL GREETING: "${getFullGreeting()}". Respond immediately.`;
            ...
            // Connect to a silence gain node to ensure processor runs without feedback
            ...
            processor.onaudioprocess = (e) => {
              ...
              // Local barge-in detection (VAD)
              if (isModelSpeakingRef.current && !isInterruptedRef.current) {
                let sum = 0;
                for (let i = 0; i < inputData.length; i++) {
                  sum += inputData[i] * inputData[i];
                }
                const rms = Math.sqrt(sum / inputData.length);
                // Threshold for speech detection (0.015 - 0.02 is usually good)
                if (rms > 0.018) {
                  console.log("Local barge-in detected (RMS:", rms, "), stopping assistant audio.");
            ...
            try {
              ...
              if (message.serverContent?.modelTurn?.parts) {
                if (isInterruptedRef.current) {
                  console.log("Ignoring model audio because of active local interruption.");
                  return;
                }
              ...
              if (message.serverContent?.inputTranscription) {
                ...
                // Safety reset: if we've been in interrupted state for > 500ms and we're getting new input,
                // the server has definitely moved on to the new turn.
                if (isInterruptedRef.current && Date.now() - lastInterruptionTimeRef.current > 500) {
                  console.log("Safety reset of interruption state due to new input transcription.");
                  isInterruptedRef.current = false;
                }
              ...
              if (message.serverContent?.turnComplete) {
                ...
                setMessages(prev => {
                  const existingIndex = prev.findIndex(m => m.id === 'live-current');
                  if (existingIndex !== -1) {
              ...
              if (message.serverContent?.modelTurn?.parts) {
                if (isInterruptedRef.current) {
                  console.log("Ignoring model transcription because of active local interruption.");
                  return;
                }
                lastActivityRef.current = Date.now();
                hasPromptedRef.current = false;
                ...
                if (textParts) {
                  ...
                  setMessages(prev => {
                    ...
                    if (existingIndex !== -1) {
                      ...
                      newMessages[existingIndex] = { 
                        ...newMessages[existingIndex], 
                        content: newMessages[existingIndex].content + textParts 
                      };
                      return newMessages;
                    }
                    return [...prev, {
  ...
  const stopAudioPlayback = (isInterruption = false) => {
    ...
    if (isInterruption) {
      console.log("Abandoning current model turn due to interruption.");
      isInterruptedRef.current = true;
      lastInterruptionTimeRef.current = Date.now();
      currentTurnRef.current.modelText = '';
      setMessages(prev => prev.filter(m => m.id !== 'live-current'));
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border-none rounded-xl text-sm text-black focus:ring-2 focus:ring-indigo-500 transition-all"
              value={billingFilters.search}
              onChange={(e) => setBillingFilters({...billingFilters, search: e.target.value})}
            />
          </div>
          <select 
            className="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm text-black focus:ring-2 focus:ring-indigo-500 transition-all"
          ...
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold uppercase tracking-widest text-black ml-1">Start Date</span>
            <input 
              type="date" 
              className="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm text-black focus:ring-2 focus:ring-indigo-500 transition-all"
          ...
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold uppercase tracking-widest text-black ml-1">End Date</span>
            <input 
              type="date" 
              className="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm text-black focus:ring-2 focus:ring-indigo-500 transition-all"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-black focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
            />
          </div>
          <select 
            value={filters.role}
            onChange={e => setFilters({ ...filters, role: e.target.value })}
            className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-black"
          >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 text-sm md:text-base text-black" 
                    />
                  </div>
                  <div>
