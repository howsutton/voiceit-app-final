...
export async function printSessionReceipt(sessionId: string) {
  ...
  try {
    if (!navigator.usb) {
      console.warn("WebUSB is not supported or allowed in this environment.");
      return; // Silently fail as requested for non-disruptive behavior
    }
    ...
    try {
      ...
      if (e.name === 'SecurityError' || msg.includes('permissions policy') || msg.includes('disallowed')) {
        console.warn("USB access disallowed by permissions policy (likely running in an iframe).");
        return;
      }
      throw e;
    }
    ...
    if (!device) {
      ...
      try {
        ...
        const msg = e.message?.toLowerCase() || "";
        if (e.name === 'SecurityError' || msg.includes('permissions policy') || msg.includes('disallowed')) {
          console.warn("USB requestDevice disallowed by permissions policy.");
          return;
        }
